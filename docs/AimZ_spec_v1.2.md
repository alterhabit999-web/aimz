...
| 部署一覧 | サイドナビ | `/departments` | 所属部署・チームへのナビゲーション |
| 案件一覧 | サイドナビ | `/projects` | 閲覧可能な全案件 |
| 案件詳細 | 遷移先 | `/projects/:projectId` | ガントチャート / カンバン / ファイル / タスク一覧 |
| タスク詳細 | モーダル or 別画面 | （モーダル） | 親タスク詳細・小タスク一覧 |
| 通知 | サイドナビ | `/notifications` | アプリ内通知 |
| Admin ダッシュボード | サイドナビ（Admin のみ） | `/admin` | 組織管理・全体ダッシュボード |
...
- 5 つのウィジェットをグリッドで配置（auto-fit minmax 360px）
  - 本日のスケジュール（タイムライン形式）
  - お知らせ（未読バッジ・件数表示）
  - 期限が迫るタスク（超過/3日以内/7日以内で色分け）
  - 担当プロジェクト（プログレスバー付き）
  - 自分の担当タスク（ステータス別グルーピング、横長カード）
...
│   │   │   ├── Avatar.jsx
│   │   │   ├── SectionLabel.jsx
│   │   │   ├── Badge.jsx                ステータス・優先度バッジ
│   │   │   ├── Button.jsx               primary / secondary / danger / ghost
│   │   │   └── PlaceholderPage.jsx      未実装ページ用
│   │   │
│   │   ├── layout/
│   │   │   ├── AppShell.jsx             サイドバー＋ヘッダーの枠組み
│   │   │   ├── Sidebar.jsx              NavLink 対応
│   │   │   ├── Header.jsx               通知バッジ付き
│   │   │   └── RequireAuth.jsx          認証必須ガード
│   │   │
│   │   └── dashboard/
...
│   │
│   └── pages/
│       ├── LoginPage.jsx                Appwrite Auth + ダミーログイン
...
│
├── docs/
│   ├── AimZ_spec_v1.2.md                本仕様書
│   └── AimZ_handover_v4.md              引き継ぎ書
│
├── .env                                 Appwrite 接続情報（Git 除外）
├── .env.example                         .env のテンプレート
├── .gitignore                           設定済み
├── DESIGN.md                            SmartHR デザインシステム詳細
├── package.json                         react / react-router-dom / appwrite / lucide-react / gh-pages
├── package-lock.json
...
- [ ] 部署・チーム・案件の画面構成・遷移パターン（次回ヒアリング予定）
...
| v1.2 | 2026-04-29 | react-router-dom v7 導入、ファイル分割（components / pages / contexts / styles / data / utils）、ダッシュボード全 5 ウィジェット実装、画面 URL 一覧追加、ファイル構成図を最新化 |
