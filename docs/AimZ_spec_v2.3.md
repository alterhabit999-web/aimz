...
> v2.2 → v2.3 の差分：
>   - 3-2 ユーザー管理：**ハード削除を廃止しソフト削除（停止）に統一**
>     * 「削除」ボタンを「停止」（Ban アイコン / danger）に変更
>     * 停止 = `profiles.is_active=false` + チーム所属を解除
>     * 停止後の Auth 完全削除は Appwrite Console での手動操作（運用ルール化）
>     * 同じメールで再登録する場合は Console での Auth 削除が必須
>   - 3-1 認証：**停止中ユーザーのログイン拒否**を実装
>     * `is_active=false` の profile を持つユーザーは login 時にエラー
>     * セッション中に停止された場合も RequireAuth で自動ログアウト
>   - **Document-level Security への移行は将来対応として明文化**
>     （社内クローズド利用の現状ではコレクション権限 + クライアントガードで継続）
>
> v2.1 → v2.2 の差分：
>   - 3-8 ガントチャート：**予定（schedules）もタイムライン上に表示**
>     * タスク行の下に区切り + 「予定」セクションラベル → 予定行を並べる
>     * 予定バーはタスクと差別化（斜線パターン + 枠線、accent カラー）
>     * クリックで ScheduleFormModal を開いて編集（ガント上でのドラッグ移動は不可）
>     * タイムラインは予定の日付範囲も含めて両端を自動拡張
>
> v2.0 → v2.1 の差分：
...
| v2.2 | 2026-05-11 | **ガントチャートに予定（schedules）も表示**：タスク行の下に区切り + 「予定」セクションを追加し、accent カラーの枠線 + 斜線パターンでタスクと差別化したバーで描画。クリックで ScheduleFormModal を開いて編集。ガント上でのドラッグ移動は不可（時刻情報の損失を避けるため、編集はモーダル経由のみ）。タイムラインは予定の日付範囲も含めて両端を自動拡張。 |
| v2.3 | 2026-05-12 | **ユーザー削除をソフト削除（停止）に統一**：UsersPage のハード削除を撤去し、停止（is_active=false + チーム所属解除）に変更。停止中ユーザーは login 時 + セッション復元時に拒否、セッション中に停止された場合は RequireAuth で自動ログアウト。Auth ユーザーの完全削除は Appwrite Console での手動操作と運用ルール化（Web SDK の制約で users.delete が不可、Appwrite Functions 化は将来対応）。Document-level Security への移行も将来対応として明文化。 |
