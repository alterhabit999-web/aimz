...
| セクション | 内容 |
|----------|------|
| **チーム一覧** | 所属部署内のチームをカード表示。部署別にグルーピング |
| **メンバー一覧** | 所属部署内のメンバーをテーブル表示。氏名・所属チーム・ロール・メール。検索フィルター付き |

ヘッダー右に「チーム作成」ボタン（Admin / Team Leader のみ表示）。  
チームカードには：チーム名、所属部署、メンバー数、リーダー名、メンバーアバター列。

### 3-12. 管理者ダッシュボード（Admin のみ）
...
| 5 | 部署管理 | 部署の CRUD（`/admin/departments`、v1.3 で追加） |
...
### 3-14. 新規作成 UI（v1.3 で確定）

すべての作成 UI は **モーダル** で実装。入力フォームは最初から全項目を展開表示する。

| 対象 | 場所 | 権限 |
|------|------|------|
| 部署 | `/admin/departments` のモーダル | Admin のみ |
| チーム | `/teams` のモーダル | Admin / Team Leader |
| 案件 | `/projects` または `/projects/:id` のモーダル | Admin / 該当チームのメンバー |
| タスク | `/projects/:id` のモーダル | Admin / 該当チームのメンバー |
...
| 案件一覧 | サイドナビ | `/projects` | 閲覧可能な全案件をチーム別グルーピング |
| 案件詳細 | 遷移先 | `/projects/:projectId` | ガント / カンバン / ファイル / タスク一覧 |
| チーム | サイドナビ | `/teams` | チーム一覧 + メンバー一覧（縦並び） |
...
| 部署管理 | サイドナビ（Admin のみ） | `/admin/departments` | 部署 CRUD（v1.3 で追加） |
| タスク詳細 | モーダル | （案件詳細から） | 親タスク詳細・小タスク一覧 |
...
#### 案件一覧
- チーム別にグルーピング表示
- 案件カード：案件名・ステータス・期間・進捗率・担当者アバター・優先度

#### 案件詳細
- ヘッダー：案件名・ステータス・期間・担当者・進捗率
- タブ切り替え：ガント（デフォルト） / カンバン / タスク一覧 / ファイル
- 編集 / 削除アクションは右上
...
│   │   ├── layout/
│   │   │   ├── AppShell.jsx
│   │   │   ├── Sidebar.jsx              v1.3 で再設計（部署表示・項目順）
│   │   │   ├── Header.jsx
│   │   │   └── RequireAuth.jsx
│   │   │
│   │   ├── dashboard/
...
│   │   │
│   │   └── teams/                       v1.3 新規
│   │       ├── TeamCard.jsx             チーム一覧カード
│   │       ├── MembersTable.jsx         メンバー一覧テーブル
│   │       └── CreateTeamModal.jsx      チーム作成モーダル
│   │
│   └── pages/
│       ├── LoginPage.jsx
│       ├── DashboardPage.jsx            ✅ ウィジェット 5 種実装済
│       ├── TeamsPage.jsx                ✅ v1.3 実装済（チーム一覧 + メンバー一覧）
...
│           └── AdminDepartmentsPage.jsx ⏸ プレースホルダー（v1.3 新規）
│
├── docs/
│   ├── AimZ_spec_v1.3.md                本仕様書
...
| v1.3 | 2026-04-29 | チームリーダーロール新設（`team_members.role`）、複数部署所属対応、サイドバー再設計（部署表示・項目順）、`/teams` 画面実装（チーム一覧+メンバー一覧、縦並び）、チーム作成モーダル実装、`/admin/departments` ルート追加、共通 Modal/FormField コンポーネント追加 |
